...
<body class="no-skin">
					...
					<div class="row">
										...
										<#list fieldList as var>
											...
											<td class="center">
												...
												<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
													...
													<a class="btn btn-minier btn-success" title="编辑" onclick="edit('${r"${var."}${objectNameUpper}_ID${r"}"}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${r"${QX.del == 1 }"}">
													<a class="btn btn-minier btn-danger" onclick="del('${r"${var."}${objectNameUpper}_ID${r"}"}');">
						...
						<table style="width:100%;">
							<tr>
								...
								<td style="vertical-align:top;"><div class="pagination" style="float: right;padding-top: 0px;margin-top: 0px;">${r"${page.pageSimpleStr}"}</div></td>
							</tr>
						</table>
						</div>
						</form>
	...
	<script type="text/javascript">
		...
		function goView(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="查看";
			 diag.URL = '<%=basePath%>${objectNameLower}/goView.do?${objectNameUpper}_ID='+id;
			 diag.Width = 452;
			 diag.Height = 300;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
