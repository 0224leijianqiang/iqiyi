								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										...
										<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
											...
											<c:if test="${QX.edit == 1 }">
											<a style="cursor:pointer;" class="green" onclick="edit('${var.PICTURES_ID}');" title="Edit">
												<i class="ace-icon fa fa-pencil bigger-130"></i>
											</a>
											</c:if>
											&nbsp;
											<c:if test="${QX.del == 1 }">
											<a style="cursor:pointer;" class="red" onclick="del('${var.PICTURES_ID}','${var.PATH}');" title="Delete">
				...
				<table style="width:100%;">
					<tr>
						<td style="vertical-align:top;">
							...
							<c:if test="${QX.del == 1 }">
							<a title="Batch Delete" class="btn btn-sm btn-danger" onclick="makeAll('Are you sure you want to delete the selected data ?');" ><i class='ace-icon fa fa-trash-o bigger-120'></i></a>
							</c:if>
						</td>
	...
	<script type="text/javascript">
		...
		function makeAll(msg){
			if(confirm(msg)){ 
					...
					if(str==''){
						alert("Did not select any content"); 
						return;
					}else{
