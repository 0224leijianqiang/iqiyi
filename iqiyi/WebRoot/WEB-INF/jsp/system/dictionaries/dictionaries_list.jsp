...
<body class="no-skin">
					...
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="Enter search keywords" class="nav-search-input" id="keywords" name="keywords" autocomplete="off" value="${page.pd.keywords }" />
								...
								<c:if test="${QX.cha == 1 }">
								<td style="vertical-align:top;padding-left:2px"><a class="btn btn-light btn-xs" onclick="gsearch();"  title="Search"><i id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue"></i></a></td>
								</c:if>
							</tr>
						</table>
						<!-- Search  -->
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="Edit" onclick="edit('${var.DICTIONARIES_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="Edit"></i>
													</a>
													<a class="btn btn-xs btn-warning" title="绑定" onclick="editTables('${var.DICTIONARIES_ID}');">
														<i class="ace-icon fa fa-retweet bigger-120" title="绑定"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.DICTIONARIES_ID}');" class="tooltip-error" data-rel="tooltip" title="Delete">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-sm btn-success" onclick="add('${DICTIONARIES_ID}');">Added</a>
									</c:if>
	...
	<script type="text/javascript">
		...
		//Delete
		function del(Id){
			bootbox.confirm("确定要Delete吗?", function(result) {
				if(result) {
					top.jzts();
					...
					$.get(url,function(data){
						...
						if("success" == data.result){
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>Delete失败！请先Delete子级或Delete占用资源.</span>",
								buttons: 			
								{
									"button" :
									{
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>Delete失败！排查表不存在或其表中没有BIANMA字段.</span>",
								buttons: 			
								{
									"button" :
									{
		...
		function editTables(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="绑定";
			 diag.URL = '<%=basePath%>dictionaries/goBind.do?DICTIONARIES_ID='+Id;
			 diag.Width = 500;
			 diag.Height =230;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
