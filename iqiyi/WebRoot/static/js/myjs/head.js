...
function viewUser(USER_ID){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = locat+'/user/viewu.do?USER_ID='+USER_ID;
	 diag.Width = 430;
	 diag.Height = 350;
...
function fhsms(){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = locat+'/fhsms/list.do?TYPE=1';
	 diag.Width = 800;
	 diag.Height = 580;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function onRes(RES_ID){
	if(RES_ID==''){
		top.Dialog.alert("暂无附件");
		return;
	}
	 jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="查看附件";
	 diag.URL = locat+'/resources/list.do?RESOURCES_ID='+RES_ID;
	 diag.Width = 500;
	 diag.Height = 75;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
