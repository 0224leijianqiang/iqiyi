...
public class Page {
	...
	private int showCount; //每 Page显示记录数
	private int totalPage;		//总 Page数
	private int totalResult;	//总记录数
	private int currentPage;	//当前 Page
	private int currentResult;	//当前记录起始索引
	private boolean entityOrField;	//true:需要分 Page的地方，传入的参数就是Page实体；false:需要分 Page的地方，传入的参数所代表的实体拥有Page属性
	private String pageStr;		//最终 Page面显示的底部翻 Page导航，详细见：getPageStr();
	private String pageSimpleStr;
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			if(currentPage==1){
				sb.append("	<li><a>Has <font color=red>"+totalResult+"</font> Item</a></li>\n");
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:50px;text-align:center;float:left\" placeholder=\" Page码\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn btn-mini btn-success\">跳转</a></li>\n");
				sb.append("	<li><a>Home Page</a></li>\n");
				sb.append("	<li><a>Prev</a></li>\n");
			}else{
				sb.append("	<li><a>Has <font color=red>"+totalResult+"</font> Item</a></li>\n");
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:50px;text-align:center;float:left\" placeholder=\" Page码\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn btn-mini btn-success\">跳转</a></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"nextPage(1)\">Home Page</a></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"nextPage("+(currentPage-1)+")\">Prev</a></li>\n");
			}
			int showTag = 5;//分 Page标签显示数量
			int startTag = 1;
			...
			sb.append("	<li><a>Has "+totalPage+" Page</a></li>\n");
			sb.append("	<li><select title='显示 Item数' style=\"width:55px;float:left;margin-top:1px;\" onchange=\"changeCount(this.value)\">\n");
	...
	public String getPageSimpleStr() {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			sb.append("	<ul style=\"height:30px;\" class=\"pagination  no-margin\">\n");
			if(currentPage==1){
		...
		pageSimpleStr = sb.toString();
		return pageSimpleStr;
	}

	public void setPageSimpleStr(String pageSimpleStr) {
		this.pageSimpleStr = pageSimpleStr;
	}
