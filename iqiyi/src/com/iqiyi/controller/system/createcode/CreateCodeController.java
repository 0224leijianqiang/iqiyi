package com.iqiyi.controller.system.createcode;
...
import com.iqiyi.service.system.createcode.CreateCodeManager;
import com.iqiyi.util.AppUtil;
import com.iqiyi.util.DateUtil;
import com.iqiyi.util.DelAllFile;
import com.iqiyi.util.FileDownload;
import com.iqiyi.util.FileZip;
import com.iqiyi.util.Freemarker;
...
public class CreateCodeController extends BaseController {
	...
	public void proCode(HttpServletResponse response) throws Exception{
		...
		System.out.println(pd.get("FIELDLIST").toString().split("&&").length);
		for(String str:pd.get("FIELDLIST").toString().split("&&")){
		...
		Freemarker.printFile("jsp_view_Template.ftl", root, "jsp/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName.toLowerCase()+"_view.jsp", filePath, ftlPath);
		...
		/*生成的全部代码压缩成zip文件*/
		if(FileZip.zip(PathUtil.getClasspath()+"admin/ftl/code", PathUtil.getClasspath()+"admin/ftl/"+objectName+".zip")){
			/*下载代码*/
			FileDownload.fileDownload(response, PathUtil.getClasspath()+"admin/ftl/"+objectName+".zip", objectName+".zip");
		}
	}
