package com.iqiyi.controller.system.login;
...
import com.iqiyi.entity.system.Menu;
import com.iqiyi.entity.system.Role;
import com.iqiyi.entity.system.User;
import com.iqiyi.service.system.appuser.AppuserManager;
import com.iqiyi.service.system.buttonrights.ButtonrightsManager;
import com.iqiyi.service.system.fhbutton.FhbuttonManager;
...
import com.iqiyi.util.RightsHelper;
...
public class LoginController extends BaseController {
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
					    ...
					    try { 
					    	...
					    	errInfo = "Authentication Defeat！";
					    }
					}else{
						errInfo = "usererror"; 				//用户名或密码有误
						logBefore(logger, USERNAME+" Login System Passord Or Username Error");
					}
				}else{
				...
				if(Tools.isEmpty(errInfo)){
					...
					logBefore(logger, USERNAME+" Login System");
					addLog(USERNAME+" Login System");
 				}
			}
		}else{
			errInfo = "error";	//缺少参数
		...
		try{
			...
			if (user != null) {
				...
				if(null == session.getAttribute(USERNAME + Const.SESSION_QX)){
					session.setAttribute(USERNAME + Const.SESSION_QX, this.getUQX(USERNAME,role));	//按钮权限放到session中
				}
	...
	public ModelAndView logout(){
		...
		logBefore(logger, USERNAME+" Logout System");
		addLog(USERNAME+" Logout System");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
	 ...
	 * @return
	 */
	public Map<String, String> getUQX(String USERNAME,Role role){
		PageData pd = new PageData();
		Map<String, String> map = new HashMap<String, String>();
		try {
			...
			Role role2 = roleService.getRoleById(role.getROLE_ID());
			Jurisdiction.getSession().setAttribute(Const.SESSION_ROLE, role2);
			if("admin".equals(USERNAME)||"0".equals(role2.getPARENT_ID())){
