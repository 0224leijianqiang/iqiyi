package com.iqiyi.controller.system.role;
...
public class RoleController extends BaseController {
	...
	public ModelAndView list()throws Exception{
		...
		try{
			pd = this.getPageData();
			if(Jurisdiction.getUserRole().equals("1")){
				
			}
			if(pd.getString("ROLE_ID") == null || "".equals(pd.getString("ROLE_ID").trim())){
				pd.put("ROLE_ID", Jurisdiction.getUserRole());										//默认列出第一组角色(初始设计系统用户和会员组不能删除)
			}
			...
			mv.addObject("pd", pd);
			if(Jurisdiction.getUserRole().equals("1")&&Jurisdiction.getUsername().equals("admin")){
				mv.addObject("roleList", roleList);
			}
			mv.addObject("roleList_z", roleList_z);
			mv.addObject("user", Jurisdiction.getUsername());
	...
	public ModelAndView add()throws Exception{
		...
		try{
			...
			roleService.add(pd);
			addLog("新增角色"+pd.getString("ROLE_ID"));
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public ModelAndView edit()throws Exception{
		...
		addLog(Jurisdiction.getUsername()+"修改角色"+pd);
		mv.setViewName("save_result");
		return mv;
	}
	...
	public Object deleteRole(@RequestParam String ROLE_ID)throws Exception{
		...
		addLog(Jurisdiction.getUsername()+"删除角色"+pd);
	...
	public void saveMenuqx(@RequestParam String ROLE_ID,@RequestParam String menuIds,PrintWriter out)throws Exception{
		...
		addLog(Jurisdiction.getUsername()+"修改菜单权限");
		PageData pd = new PageData();
		try{
